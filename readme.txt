...
= 1.9.5 = (2026-04-05)
* Changed: nascosta automaticamente su pagine flusso FP Experiences (prenotazione); filtro `fp_cta_bar_hide_on_fp_experience_pages`.

= 1.9.4 = (2026-04-05)
* Changed: menu admin raggruppato con gli altri plugin FP (posizione 56.15).

= 1.9.3 = (2026-04-04)
* Fixed: nome evento da impostazioni GTM/GA4 applicato al dataLayer (FP Tracking).
* Changed: testo guida admin su parametri dataLayer e dimensioni GA4.

= 1.9.2 = (2026-04-04)
* Changed: Click tracciati via GTM/client (`fpCtaBarClick`); REST non invia più enqueue server-side verso il layer. `fp_cta_bar_clicked` espone anche category ed event_id.

= 1.9.1 = (2026-04-04)
* Added: `event_id` allineato tra beacon e CustomEvent; enqueue server `cta_bar_click` (rimosso in 1.9.2 a favore del solo client/GTM).

= 1.9.0 = (2026-03-24)
* Added: Nuova pagina admin "Statistiche" con KPI click (totale, oggi, ultimi 7 giorni), tabella top link e reset dati.
* Added: Persistenza aggregata click su `wp_options` tramite action `fp_cta_bar_clicked` (label, URL, lingua, primo/ultimo click).
* Fixed: Allineamento icona + testo nel bottone CTA (`inline-flex` + `gap`) in modalità bottone.

= 1.8.4 = (2026-03-19)
* Fixed: Layout checkbox «Traccia click» in admin
* Changed: Eventi verso tracking layer solo per link con tracciamento attivo

= 1.8.3 = (2026-03-19)
* Fixed: Anteprima impostazioni contenuta nel box (non più agganciata al footer admin)

= 1.8.2 = (2025-03-19)
* Fixed: Notice di terze parti non finiscono più dentro il banner (h1 screen-reader + titolo visibile h2; compat jQuery .wrap h1)

= 1.8.1 = (2026-03-19)
* Fixed: Notice admin WordPress di nuovo sopra il contenuto (CSS flex/order rimosso)

= 1.8.0 = (2026-03-19)
* Changed: Icona principale = solo SVG esplicativi; link = emoji + WhatsApp
* Added: IconSvg::settings_main_icon_options(); settings_link_icon_options() per i link

= 1.7.1 = (2026-03-19)
* Added: WhatsApp (logo colorato) nel selettore icone

= 1.7.0 = (2026-03-19)
* Added: Molte emoji in più nel preset
* Changed: Griglia icone solo emoji (+ URL immagine manuale); vecchie chiavi salvate restano valide

= 1.6.0 = (2026-03-19)
* Added: Preset emoji Unicode (colori dipendono da OS/browser)
* Changed: Griglia icone snellita; logo brand SVG non più in elenco (restano validi se già salvati)
...
= 1.4.1 = (2026-03-19)
* Fixed: Admin — griglia icone non più tagliata dalle card; z-index sopra altre righe/elementi
* Changed: Admin — griglia icone più compatta (6 colonne), etichette su 2 righe, dropdown più largo nel repeater link

= 1.4.0 = (2026-03-19)
* Added: Icone preset come SVG line-art sul sito (più curate delle Dashicons); admin allineato con anteprima SVG
* Changed: Stile pannello link (tile icona, padding, angoli)
...
= 1.3.7 = (2026-03-19)
* Changed: Stile bottone CTA flottante più pulito (ombre morbide, bordo leggero, testo senza forzare maiuscole); barra full-width allineata (etichetta e ombra)

= 1.3.6 = (2026-03-19)
* Fixed: Bottone CTA su mobile (Safari/iOS): rimosso filter sul wrapper fixed, ombra sul bottone, safe-area e hint dispositivo in admin
* Changed: Barra full-width con padding-bottom per safe-area (notch / home indicator)

= 1.3.5 = (2026-03-19)
* Changed: Più spazio nel pannello e nei link per evitare testi tagliati o troppo stretti

= 1.3.4 = (2026-03-19)
* Changed: Rifinitura estetica frontend (pannello, link, bottone, full-width, accessibilità movimento ridotto)

= 1.3.3 = (2026-03-19)
* Added: Colore separato per testo e icone nel pannello link (indipendente da barra/bottone principale)
* Changed: Migrazione automatica del vecchio colore testo nel nuovo campo pannello
